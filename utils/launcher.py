        ...
        if settings.USE_UA:
            with open('config/user-agent.txt', 'r') as file:
                ...
                if user_data['count_user'] > i:
                    ...
                    logger.success(
                        f"System | List <c>[{i}]</c> user-agent download...")
                    return True
        else:
            for i in range(user_data['count_user']):
                user_data[i]['user-agent'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36 Edg/127.0.0.0'
        ...
        if settings.USE_PROXY:
            with open('config/proxies.txt', 'r') as file:
                ...
                if user_data['count_user'] > i:
                    ...
                    logger.success(
                        f"System | List <c>[{i}]</c> proxy  download...")
                    return True
        else:
            for i in range(user_data['count_user']):
